...
app.post("/register", async (req, res) => {
  ...
  try{
    const checkEmailExists = await db.query("SELECT * FROM users WHERE email = $1",
      [email]
    );
...
app.post("/login", async (req, res) => {
  ...
  const password = req.body.password;
  try{
    const output= await db.query("SELECT * FROM users WHERE email = $1 AND password = $2", [email, password]);
